...
5. **⚠️ 분량 엄수**: 각 섹션(mulsangron, psychology, counseling)은 **300자 이내**로 핵심만 짚어줘. 전체 JSON 응답은 **1,500자 이내**로 마무리할 것.
6. **⚠️ JSON 완결 필수**: 반드시 JSON이 완전히 닫힌 상태로 응답을 끝낼 것. 분량을 맞추기 위해 내용을 줄이되 절대 JSON 구조를 열어둔 채 끊지 말 것.
7. 반드시 아래 JSON 스키마 형식으로만 응답할 것.
...
// 상성 분석 프롬프트
// ────────────────────────────────────────────
export const RELATIONSHIP_REPORT_PROMPT = (
...
// 무료 사주 시스템 프롬프트
// ────────────────────────────────────────────
export const FREE_SYSTEM_PROMPT = (
  year: number,
  month: number,
  name: string,
  raw: import("@/types").SajuRawResult,
...
export function buildSectionGuide(
  year: number,
  name: string,
...
// 유료 사주 시스템 프롬프트
// ────────────────────────────────────────────
export const PAID_SYSTEM_PROMPT = (
  year: number,
  month: number,
  name: string,
  raw: import("@/types").SajuRawResult,
